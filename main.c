...
/**
 * Ce programme montre un cas d'utilisation de la memoire partagee de MPI pour calculer la somme des
 * des carres des n (n etant le nombre de processeur utilises) premiers entiers strictement positifs.
 */
...
int main(int argc, char *argv[]) {
    ...
    if (rank != 0) {
        // Les processeurs de rang different de 0 recuperent chacun un pointeur vers la memoire allouée par le processeur de rang 0
    ...
    if (rank == 0) {
        // Exclusivite de l'acces a la memoire partagée au processeur de rang 0
        MPI_Win_lock(MPI_LOCK_EXCLUSIVE, 0, 0, win);
        for (i = 0; i < size; i++) {
            shared[i] = i + 1;
        }
        MPI_Win_unlock(0, win);// Deverouilage de l'acces a la memoire
    }
    ...
    shared[rank] = shared[rank] * shared[rank];
    MPI_Barrier(MPI_COMM_WORLD);
